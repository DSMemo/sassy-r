...
sas.ttables <- function(dataset,alpha = .05){
  ...
  highstddev <- sqrt((sd(dataset)^2*length(dataset)-1)/ qchisq(p = (alpha/2),df = length(dataset)-1))
  
  lowstddev <- sqrt((sd(dataset)^2*length(dataset)-1) / qchisq(p = 1- (alpha/2),df = length(dataset)-1))
